...
const getRevenueEvolution = async ()=>{
  orderEvolution.value = await  statsStore.getTotalRevenueEvolution(props.shopId,props.period);
}
...
const initChart = () => {
  if (orderEvolution.value) {
    ...
    if (document.getElementById("revenue-chart") && typeof ApexCharts !== 'undefined') {
      chart.value = new ApexCharts(document.getElementById("revenue-chart"), options.value);
      chart.value.render();
    }
  }
}
...
<template>
  <p class="font-poppins font-bold text-heading-2 m-4">Evolution des revenues</p>
